...
test('connects an agent, opens its fleet and a ship, then signs out', async ({ page }) => {
  ...
  await expect(
    page.locator('main').getByText('TEST', {
      exact: true,
    }),
  ).toBeVisible()
  ...
  const fleetTable = page.getByRole('table', {
    name: 'Fleet',
    exact: true,
  })
  ...
  await expect(fleetTable.locator('tbody tr')).toHaveCount(2)

  await expect(
    page.getByRole('link', {
      name: 'Open ship TEST-2',
      exact: true,
    }),
  ).toBeVisible()

  // The ship link in the table opens its details.
  await page
    .getByRole('link', {
      name: 'Open ship TEST-1',
      exact: true,
    })
    .click()
  ...
  await expect(
    page.getByText('Cargo hold is empty', {
      exact: true,
    }),
  ).toBeVisible()
